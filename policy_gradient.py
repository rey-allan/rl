...
class REINFORCEWithBaseline:
    ...
    def learn(self, epochs=200, alpha_policy=0.01, alpha_value=0.01, gamma=0.9, verbose=False, **kwargs) -> np.ndarray:
        ...
        # Value function
        w = np.random.rand(36)
        value_approximator = lambda s: [np.dot(encode(s, a), w) for a in [Action.hit, Action.stick]]
        # Policy function
    ...
    def _sample_episode(self, pi: Callable[[State, Action, np.ndarray], float], theta: np.ndarray) -> List[Trajectory]:
        ...
        s = self._env.reset()
        # The policy selects the action with some constant exploration as oin the Easy21 assignment
        policy = (
